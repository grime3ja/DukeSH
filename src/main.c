...
{
  // argc = 1 (./dukesh), take commands and arguments from stdin
  // argc = 3 (./dukesh -b {file}), take commands and arguments from a
  // specified file
  if (argc != 1 && argc != 3)
    {
      usage ();
  ...
  // default file stream to stdin
  FILE *file = stdin;

  // do we need this if statement? i think it will work without it
  // updates filestream if -b argument is passed with ./dukesh
  if (!get_args (argc, argv, &file))
    {
    }
  // calls shell.c:shell function
  shell (file);
  return EXIT_SUCCESS;
}
